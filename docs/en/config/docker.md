...
| `CODEBUDDY_SESSION_LOG_ENABLED`           | Records session logs                                             | `true` (default); database backends only                    |
| `CODEBUDDY_SESSION_RETENTION_DAYS`        | Session log retention                                            | `30` days; `0` disables recording                           |
| `CODEBUDDY_SESSION_MAX_TURNS_PER_SESSION` | Maximum turns kept per session                                   | `200`; `0` keeps every turn                                 |
