...
| `CODEBUDDY_USAGE_RETENTION_DAYS`          | 使用量イベントの保持日数（クォータ集計期間）       | `35`（file バックエンドは `7`）、クォータ期間以上が必要 |
| `CODEBUDDY_QUOTA_ENFORCEMENT`             | クォータ超過時の挙動                               | `reject`（既定）/ `warn` はログのみ                     |
| `CODEBUDDY_SESSION_LOG_ENABLED`           | セッションログを記録するか                         | `true`（既定）。DB バックエンドのみ                     |
| `CODEBUDDY_SESSION_RETENTION_DAYS`        | セッションログの保持日数                           | `30` 日。`0` は記録しない                               |
| `CODEBUDDY_SESSION_MAX_TURNS_PER_SESSION` | 1 セッションで保持する最大ターン数                 | `200`。`0` は無制限                                     |
